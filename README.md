...
compatmalloc delivers **hardened-allocator security at near-glibc throughput** -- 4x faster than Scudo with comparable protections.
...
A multi-stage [Dockerfile](Dockerfile) is provided for zero-friction container integration:

```dockerfile
# In your application's Dockerfile:

# Option 1: Use the artifact stage (just the .so file)
# First build: docker build --target artifact -t compatmalloc-artifact .
COPY --from=compatmalloc-artifact /libcompatmalloc.so /usr/lib/libcompatmalloc.so
ENV LD_PRELOAD=/usr/lib/libcompatmalloc.so

# Option 2: Use the hardened-base stage as your base image
# docker build --target hardened-base -t my-hardened-app .
```

Or use the ready-made hardened base directly:
